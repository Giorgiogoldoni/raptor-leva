...
def process_ticker(info):
    ...
    try:
        tk = yf.Ticker(symbol)
        hist = tk.history(period='2y', interval='1d', timeout=15)
...
def main():
    ...
    for i, info in enumerate(unique):
        ...
        if result:
            # Timestamp per-ticker: quando questo specifico file è stato generato
            result['timestamp'] = now.isoformat()
            result['timestamp_it'] = now.strftime('%d/%m/%Y %H:%M')

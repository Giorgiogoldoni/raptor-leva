...
def update_sbb_flip_log(results, now):
    """Log una riga per ogni flip Super Best Buy: si apre al flip BULL (prezzo
    ingresso = prezzo corrente), si aggiorna con l'ultimo prezzo finché il
    trend resta BULL, si chiude al flip opposto (BEAR) con rendimento%.
    Retention: righe chiuse più vecchie di SBB_LOG_RETENTION_D giorni scartate."""
    try:
        with open(SBB_LOG_FILE, 'r', encoding='utf-8') as f:
            log = json.load(f)
    except Exception:
        log = []
    ...
    for r in results:
        ...
        is_new_flip = r.get('super_best_buy') and t not in open_idx
    ...
    try:
        with open(SBB_LOG_FILE, 'w', encoding='utf-8') as f:
            json.dump(log, f, ensure_ascii=False)
    except Exception as e:
        print(f"Errore salvataggio {SBB_LOG_FILE}: {e}")
    return log
